...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createDropdownOptions(data, dropdown, placeholder) {
        ...
        data.forEach(item => {
            ...
            option.textContent = item.replace(/\w\S*/g, (w) => (w.replace(/^\w/, (c) => c.toUpperCase())));
            dropdown.appendChild(option);
        });
    }

    function calculateFee(language, purpose, proficiency) {
        const selectedLanguage = languageData.find(lang => lang.name.toLowerCase() === language);
        if (selectedLanguage) {
            const selectedLevel = selectedLanguage.levels.find(level => level.name.toLowerCase() === proficiency);
            if (selectedLevel) {
    ...
    function updateProficiencyLevels(language) {
        const levels = languageData.find(lang => lang.name.toLowerCase() === language).levels;
        ...
        document.querySelectorAll('input[name="proficiency[]"]').forEach(radio => {
            radio.addEventListener('change', () => {
                if (radio.checked) {
                    ...
                    updateDisplay('proficiency', proficiency.charAt(0).toUpperCase() + proficiency.slice(1));

                    const selectedLanguage = document.querySelector('input[name="language[]"]:checked').id.split('-')[0];
                    const selectedPurpose = document.querySelector('input[name="purpose[]"]:checked').id.split('-')[0];
                    const fee = calculateFee(selectedLanguage, selectedPurpose, proficiency);
                    document.getElementById('fee-data').querySelector('span').textContent = '₹' + fee;
                }
            });
        });
    }
    ...
    document.querySelectorAll('input[name="language[]"]').forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.checked) {
                ...
                updateDisplay('language', language.charAt(0).toUpperCase() + language.slice(1));
    ...
    document.querySelectorAll('input[name="purpose[]"]').forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.checked) {
                ...
                updateDisplay('purpose', purpose.charAt(0).toUpperCase() + purpose.slice(1));

                // Recalculate fee if proficiency is already selected
                const selectedProficiencyRadio = document.querySelector('input[name="proficiency[]"]:checked');
                if (selectedProficiencyRadio) {
                    const selectedProficiency = selectedProficiencyRadio.id.split('-')[0];
                    const selectedLanguage = document.querySelector('input[name="language[]"]:checked').id.split('-')[0];
                    const fee = calculateFee(selectedLanguage, purpose, selectedProficiency);
                    document.getElementById('fee-data').querySelector('span').textContent = '₹' + fee;
                }
            }
        });
    ...
    document.getElementById('proficiency-select').addEventListener('change', () => {
        const selectedProficiency = document.getElementById('proficiency-select').value;
        updateDisplay('proficiency', selectedProficiency.charAt(0).toUpperCase() + selectedProficiency.slice(1));
        ...
        // Recalculate fee
        const fee = calculateFee(selectedLanguage, selectedPurpose, selectedProficiency);
        document.getElementById('fee-data').querySelector('span').textContent = '₹' + fee;
    });
    ...
    document.getElementById('language-price-form').addEventListener('submit', (e) => {
        e.preventDefault();
        const name = document.getElementById('name').value;
        const email = document.getElementById('email').value;
        const phone = document.getElementById('phone').value;
        const language = window.innerWidth < 768 ? document.getElementById('language-select').value : document.querySelector('input[name="language[]"]:checked').id.split('-')[0];
        const purpose = window.innerWidth < 768 ? document.getElementById('purpose-select').value : document.querySelector('input[name="purpose[]"]:checked').id.split('-')[0];
        const proficiency = window.innerWidth < 768 ? document.getElementById('proficiency-select').value : document.querySelector('input[name="proficiency[]"]:checked').id.split('-')[0];
        ...
        fetch('https://curiotory.com/wp-json/api/v1/custom_language_price_data/', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    document.getElementById('purpose-select').value = '';
                    document.getElementById('proficiency-select').value = '';
                    document.getElementById('language-data').querySelector('span').textContent = '-';
                    document.getElementById('purpose-data').querySelector('span').textContent = '-';
                    document.getElementById('proficiency-data').querySelector('span').textContent = '-';
                    document.getElementById('fee-data').querySelector('span').textContent = '₹-';
                } else {
                    Toastify({
